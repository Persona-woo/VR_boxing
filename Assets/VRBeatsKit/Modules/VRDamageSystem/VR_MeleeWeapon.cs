...
namespace DamageSystem
{
    ...
    public class VR_MeleeWeapon : MonoBehaviour
    {
        ...
        protected bool TryDoDamage(Transform target, Vector3 hitPoint)
        {
            ...
            if (damageableArray != null && damageableArray.Length > 0)
            {
                for (int n = 0; n < damageableArray.Length; n++)
                {
                    ...
                    {
                        RaycastHit hitInfo;
                        if (Physics.Linecast(rayStart.position, rayEnd.position, out hitInfo,
        ...
        protected virtual DamageInfo CreateDamageInfo(Vector3 hitPoint)
        {
            ...
            damageInfoCache.sender = grabbable.GrabController != null
                ? grabbable.GrabController.transform.root.gameObject
                : null;
            damageInfoCache.canDismember = canDismember;

            var beat = new BeatDamageInfo(damageInfoCache);
            beat.velocity = controllerVelocity.magnitude;
            beat.colorSide = this.colorSide;
